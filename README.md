...
*   **Sandboxed Test Runner:** The `/api/run-tests` route executes generated unit tests in a sandboxed child process — esbuild bundling, Node's Permission Model (filesystem scoped to a temp dir; no child-process/worker/native-addon access), a minimal env with no platform secrets, and time/output/input-size limits. **Outbound network is blocked at the code level** — a preloaded guard (`__netguard.mjs`) neutralises `net`/`dgram`/`dns`/`fetch`/`process.binding` before the test runs, so test code cannot reach the cloud metadata endpoint or any network (DNS included, since c-ares bypasses `net.Socket`). This is defence-in-depth alongside the permission model, not a formal microVM boundary — the fully isolated runner service remains roadmap (see [`SECURITY.md`](SECURITY.md)). **Live S/4HANA test execution stays disabled unless network egress is explicitly enforced at the infrastructure level (`S4_TEST_RUNNER_EGRESS_ENFORCED=true`).** See [`SECURITY.md`](SECURITY.md).
