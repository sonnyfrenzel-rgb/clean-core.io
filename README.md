...
*   **Sandboxed Test Runner:** The `/api/run-tests` route executes generated unit tests in a sandboxed child process — esbuild bundling, Node's Permission Model (filesystem scoped to a temp dir; no child-process/worker/native-addon access), a minimal env with no platform secrets, and time/output/input-size limits. **Common outbound-network paths are blocked at the code level** — a preloaded guard (`__netguard.mjs`) neutralises `net`, `dgram` (the factory and the `dgram.Socket` constructor), `dns`, `fetch` and `process.binding` before the test runs, to block the usual egress routes to the cloud metadata endpoint and the network (DNS included, since c-ares bypasses `net.Socket`). This is **defence-in-depth, not a complete isolation boundary** — Node's permission model is explicitly not a security guarantee against malicious code, so the runner is a restricted child process, not a sandbox for untrusted code. A fully isolated runner service remains roadmap (see [`SECURITY.md`](SECURITY.md)). **Live S/4HANA test execution is locked** (gate `G0:R0`, [`SECURITY.md`](SECURITY.md) §7.1): the route refuses it before any measurement, and it reopens only with an isolated runner service, deny-by-default egress proven in CI, an external review and an owner decision. Underneath the lock the older safeguards stay in place: `S4_TEST_RUNNER_EGRESS_ENFORCED=true` states an intent; before any live run the runtime probes the cloud metadata endpoint and a public address and refuses if either answers, so a variable cannot assert a policy that is not in force. The guard is never removed either: on an attested live run it narrows to the `S4_HOST_ALLOWLIST` suffixes rather than being skipped, which keeps the metadata endpoint (an IP literal, matching no suffix) unreachable on that path too. See [`SECURITY.md`](SECURITY.md).
