 ...
 *
 * A copy of `playwright.port3100.config.ts` for the reason given there: several
 * agents run this suite against this machine at the same time, and the shared
 * config reuses whatever dev server is listening on 3000. When one run finishes
 * it stops the server the others are still using, and every spec left standing
 ...
 *
 * Use with `npx playwright test -c playwright.port4100.config.ts`. Delete this
