    ...
    if (data) {
      return (
        <div className="space-y-12">
          ...
          {/* Decoupled Target Topology Diagram */}
          <div className="bg-slate-950 text-white rounded-[2rem] p-6 sm:p-8 border border-slate-805 shadow-xl relative overflow-hidden">
            {/* Ambient glows */}
            ...
            <div className="w-full overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
              <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-2 relative z-10 md:min-w-[1100px] px-2 pb-2">
                {(() => {
                  const isAbapCloud = project?.extensibilityRoute ? !project.extensibilityRoute.includes('BTP') : false;
                  const steps = isAbapCloud ? [
                    { label: 'UI Consumer', desc: 'Fiori Elements', icon: <Globe className="w-5 h-5 text-emerald-400" /> },
                    { label: 'Service Exposure', desc: 'OData V4 Binding', icon: <Network className="w-5 h-5 text-emerald-400" /> },
                    { label: 'Business Logic', desc: 'ABAP ABP Class', icon: <Terminal className="w-5 h-5 text-emerald-400" /> },
                    { label: 'Data Modeling', desc: 'Projection CDS', icon: <Layers className="w-5 h-5 text-emerald-400" /> },
                    { label: 'ERP Standard', desc: 'Protected Core', icon: <ShieldCheck className="w-5 h-5 text-emerald-400" /> }
                  ] : [
                    { label: 'UI Consumer', desc: 'Fiori App / API Client', icon: <Globe className="w-5 h-5 text-emerald-400" /> },
                    { label: 'BTP CAP App', desc: 'Node.js Microservice', icon: <Terminal className="w-5 h-5 text-emerald-400" /> },
                    { label: 'Secure Tunnel', desc: 'BTP Destination', icon: <ShieldCheck className="w-5 h-5 text-emerald-400" /> },
                    { label: 'Released API', desc: 'Standard OData / REST', icon: <Network className="w-5 h-5 text-emerald-400" /> },
                    { label: 'ERP Core', desc: 'S/4HANA Standard', icon: <Layers className="w-5 h-5 text-emerald-400" /> }
                  ];

                  return steps.map((step, idx) => (
                    <div key={idx} className="flex flex-col md:flex-row items-center w-full md:w-auto">
                      {/* Step Pill */}
                      <div className="flex items-center gap-3 bg-white/5 border border-white/10 hover:border-emerald-500/30 hover:bg-white/10 transition-all duration-300 p-4 rounded-2xl w-full md:w-48 shadow-sm">
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      {activeTerm && (() => {
        ...
        return (
          <div 
            className="fixed inset-0 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md z-[90] animate-in fade-in duration-200"
