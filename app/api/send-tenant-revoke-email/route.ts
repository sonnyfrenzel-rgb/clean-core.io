...
import { buildTenantRevokeEmail, TENANT_REVOKE_SUBJECT } from '@/lib/tenant-email';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailSubject = TENANT_REVOKE_SUBJECT;
    ...
    const emailHtml = wrapEmailDocument(buildTenantRevokeEmail({ name, recipient: escapeHtml(email) }));
