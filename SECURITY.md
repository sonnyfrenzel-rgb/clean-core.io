...
- **What the server keeps:** `users/{uid}.mfaEnabled` and `mfaFactor`, written only by `POST /api/mfa/enrolled` after reading back from Firebase Auth that the factor exists (the client update allowlist in `firestore.rules` excludes the flag). `POST /api/mfa/disable` removes the factor in Firebase Auth through the Admin SDK — only from a session that carries it. The order is flag first, factor second, and the flag is restored if the removal fails, so an account never requires a factor it does not have. A flag without a factor in Firebase Auth (a removal whose profile write failed, or the legacy of the retired application-level TOTP) is cleared without a step-up: there is nothing left that a first-factor token could remove, only a flag refusing its own owner. If recording an enrolment fails after Firebase enrolled the factor, the Settings page records it again on its next visit — the route is idempotent and reads the factor back from Firebase Auth.
