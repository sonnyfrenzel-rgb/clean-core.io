...
- **Sandboxed test runner**: generated tests do not run inside the application service. They run in a separate runner service (a service account without roles, no platform secrets, no open network egress), inside a restricted Node process there (esbuild bundle + Node Permission Model — filesystem scoped to a temp dir, no child-process/worker/native access). Test execution against a live S/4HANA tenant is locked (gate ${LIVE_TEST_EXECUTION.id}): the live runner never receives tenant credentials, and the path reopens only after its external review.
