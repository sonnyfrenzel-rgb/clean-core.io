 ...
 * it measures is the infrastructure: from here, the fixed destinations in
 * `RUNNER_NETWORK_PROBES` (two public hosts, one private address) must be
 * unreachable in both modes. They are samples, not a proof that nothing else
 * is reachable — the boundary is the VPC without NAT and its firewall. The
 * caller sends nothing but the request.
 *
...
async function handleNetworkSelftest(res: http.ServerResponse) {
  const probes = [];
  for (const p of RUNNER_NETWORK_PROBES) probes.push({ target: p.label, reached: await reachable(p.host, p.port) });
