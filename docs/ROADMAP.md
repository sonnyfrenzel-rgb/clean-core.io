...
| 0.3 | Level-Regelseite mit Regelversion; Score umbenannt, TCO-Versprechen entfernt. **Namensprüfung „Clean Core Score"** (Frage Sonny 15.09.2026, nur Prüfung und Umsetzung hier, nicht sofort): SAP führt im RISE-with-SAP-Methodology-Dashboard in SAP Cloud ALM eigene Score-Kennzahlen im selben Feld — „Technical Debt Score", „Clean Core Share" und nach Community-Beiträgen auch einen „Clean Core Score", bei dem ein **höherer Wert mehr technische Schuld** bedeutet, also die umgekehrte Richtung. Zu klären: (1) SAPs wörtliche Benennung aus der SAP-Hilfe zum Dashboard, (2) Markenlage „Clean Core" / „Clean Core Score" (EUIPO/TMview, DPMA, USPTO), (3) neuer Produktname — Vorschlag nach `DESIGN.md` §5.6 **„Clean-Core Readiness"** mit Regelversion. Randbedingungen: `/clean-core-score` behält URL und Canonical (458 Impressionen, Position 7; `tests/seo-surface-guard.spec.ts`), Titel und Text nennen den alten Namen einmal („formerly Clean Core Score"), Umbenennung in allen 37 Dateien, die den Begriff heute führen, samt Chatbot-Wissen, `llms.txt` und Board-Deck | M |
