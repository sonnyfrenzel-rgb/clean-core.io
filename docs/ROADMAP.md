...
| 8.9 | **Isolierter Test-Runner** (CR-09, Entscheidung §9 Nr. 16): eigener Cloud-Run-Dienst oder Job mit einem Service-Konto ohne Rollen, ohne App-Secrets, eigener Dateisystem- und Netzgrenze; der Mock-Pfad läuft dort oder gar nicht; Ergebnisnachweise stammen vom isolierten Worker; ein autorisierter Negativtest auf dem Deploymentprofil erreicht weder fremde Dateien noch Zugangsdaten — bisher „nach 3.0, ohne Version". **Entschieden (Sonny, 24.09.2026): jetzt, vollständig, als Cloud Run Service; der Mock-Pfad wird bis dahin nicht gesperrt; der Live-Pfad kommt vor 3.0 zurück.** Aufbau: zwei Dienste, `clean-core-runner` (Mock) und `clean-core-runner-live`, beide mit einem Service-Konto **ohne Rollen**, ohne Secrets, Ingress intern, Aufruf nur durch die App (`run.invoker`). Ausgehend alles über ein eigenes VPC ohne NAT: der Mock-Runner erreicht nichts; der Live-Runner erreicht nur die App, über Private Google Access. **Die S/4-Zugangsdaten gelangen nie in den Runner:** er ruft den Tenant über einen Proxy der App, der nur den freigegebenen Host bedient und die Zugangsdaten selbst einsetzt — generierter Testcode kann sie nicht abgreifen, weil er sie nie hat. Die Infrastruktur legt der Projektinhaber an (Skript beim Betreiber), der Code kommt über die Pipeline | L |
...
| 3.0.8 | **Alles Öffentliche im Repository auf 3.0 ziehen — aus einem Guss, mit 3.0, nicht vorher** (Entscheidung Sonny 16.09.2026; **erweitert 24.09.2026: nicht nur die Dateien, die Gestrichenes nennen, sondern jede**). Das Repository ist öffentlich, jede eingecheckte Textdatei ist damit Produktkommunikation. **Umfang** (Stand 24.09.2026: 132 Markdown- und Textdateien): die sechs im Wurzelverzeichnis — `README.md`, `CHANGELOG.md`, `CLAUDE.md`, `DESIGN.md`, `SECURITY.md`, `THIRD-PARTY-NOTICES.md` —, die 72 unter `docs/` außerhalb von Archiv und Korpus (`ARCHITECTURE.md`, die Runbooks der drei Agenten, Konzepte, Pläne, Mails), die Skills unter `.claude/skills/`, `llms.txt` (`app/llms.txt/route.ts`), die Beschreibung in `package.json` und alles, was als Text öffentlich ausgeliefert wird. **Aus einem Guss heißt:** *ein Produktbild* (der Arbeitsraum mit drei Sichten, die sieben Stufen als Werkzeuge — 3.0.1; nirgends mehr die sieben Stufen als Produkt, die alte Startseite, Signavio-Import, Dark Mode oder andere gestrichene Teile), *ein Vokabular* (Sichten, Ebenen, Level A–D, die Herkunftswerte aus `lib/provenance.ts`, „nicht bestimmt", der Satz zur Selbstauskunft — je genau eine Schreibweise), *eine Stimme* (sachlich, belegt, ohne Eigenlob, jede Zahl mit Quelle), *ein Stand* (Version, Modelle, Umgebungen, Agenten und Abläufe stimmen überall mit dem Code überein). **Vorgehen:** Inventur mit einer Entscheidung je Datei — aktualisieren, nach `docs/archiv/` verschieben (mit Eintrag im Index) oder löschen; erledigte Konzept- und Planpapiere werden archiviert, nicht umgeschrieben; `docs/korpus/` und `docs/archiv/` sind fachliche Quelle bzw. Geschichte und werden nur auf Verweise geprüft. **Abnahme:** ein Guard prüft, dass (a) keine öffentliche Datei Elemente nennt, die 3.0 entfernt hat (Liste aus 3.0.5 und §8), (b) die Kernbegriffe je eine Schreibweise haben und (c) jede Datei außerhalb von Archiv und Korpus in der Inventur mit ihrer Entscheidung steht. Bis dahin bleiben die Texte, wie sie sind — sie beschreiben, was ausgeliefert ist. Zusammen mit den Produkttexten aus 3.0.6 | L |
...
**Vollprüfung von 81810c8026e0 (v2.18.0, 24.09.2026, `openai/gpt-5.6-sol`, 28 Aufrufe, 2,10 $):** Verdikt `no_go`, **INCOMPLETE** (rund 250 Dateien außerhalb des Budgets). Neu: 2 kritisch, 10 hoch. Kritisch `7be9e6ff7de4` (mit den übertragenen `a862eec671b7`/`ba99f286796e`) behoben — Specs mit dem Firebase-Client brechen ohne Emulator ab und nutzen Wegwerfkonten; `8f985df72110` widerlegt (Testliteral). Von den zehn hohen sind alle bestätigt: behoben `7d0d04a45324`, `d5a87a5db395`, `183ed4edf700`, `68c8263e20f6`, `bfbbcc22a9f2`, `ff8dcda705e4`, `c6b7be3306be`; eingeplant `e649177b3894`/`c42de15e9c75` (3.0.11) und `f71a57e2d326` (3.0.12). Volltext nur lokal unter `.qa-review/81810c8026e0.full.json`.
...
| 0c3362018102 | high | components/SamplePackageDownload.tsx | The advertised importable package contains an invalid CDS view-entity annotation | 0.15 | entfällt mit 3.0.6 (Showroom gestrichen, 24.09.2026) |
| 0fd415bb9927 | high | tests/security-compliance.spec.ts | Admin E2E requests cannot satisfy the enforced MFA step-up | — | widerlegt |
| 1012acfcd241 | high | components/TransformationReplay.tsx | Timer-driven replay reports compilation and test-generation results that were never produced | 0.2 | entfällt mit 3.0.6 (Showroom gestrichen, 24.09.2026) |
...
| 789f1985a410 | high | components/TransformationShowroom.tsx | Static showroom markup presents unexecuted tests and compilation as passed validations | 0.2 | entfällt mit 3.0.6 (Showroom gestrichen, 24.09.2026) |
...
| 666a399f4dd2 | high | components/TransformationReplay.tsx | Timer-driven replay still reports compilation and generated tests that never occurred | 0.2 | entfällt mit 3.0.6 (Showroom gestrichen, 24.09.2026) |
...
| fa9e39148077 | high | app/page.tsx | Landing page still calls static demonstrations compiled, tested, and verified | 0.2 | entfällt mit 3.0.6 (Showroom gestrichen, 24.09.2026) |
...
| 72556d36c205 | medium | components/SamplePackageDownload.tsx | The ABAP Unit include is named for the local test class instead of its owning global class | 0.15 | entfällt mit 3.0.6 (Showroom gestrichen, 24.09.2026) |
