...
| Öffentlich per Design | der Firebase-Web-API-Schlüssel in `firebase-config.json` wird geschwärzt, aber nicht als Geheimnis gemeldet — sonst käme er in jedem Vollreview als kritischer Befund wieder. Das hängt an **zwei** Stellen, und bis zum 22.09.2026 gab es nur eine: `isPublicByDesign` in `config.mjs` vergleicht den **Pfad** eines Treffers, was für die Schwärzung je Datei reicht. Das letzte Netz vor dem Versand meldet seine Treffer aber unter dem Pfad `outgoing message`, und die Delta-Review filterte überhaupt nicht — also stand „Possible Google API key committed" bei **jedem** Push als kritischer Befund im Bericht (Übertrag `19146e4fbb01`). Seit `publicByDesignValues()` wird zusätzlich der **Wert** verglichen, in allen drei Sendern (`review.mjs`, `full-review.mjs`, `scripts/security/audit.mjs`); `tests/qa-review-guard.spec.ts` hält fest, dass jeder andere Wert derselben Form weiterhin gemeldet wird |
