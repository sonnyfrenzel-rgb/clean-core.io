...
- **Multi-provider BYOK — model choice (Gemini + Claude)** — let users pick their AI model, adding Anthropic Claude alongside Google Gemini as a secure BYOK option. *Concept 2026-07-05:* generalize the Gemini proxy into a provider abstraction (`lib/ai/*`, canonical `/api/ai`, `/api/gemini` kept as alias) with a **per-provider server-side model allowlist**. Store keys exactly as today — `user_secrets/{uid}/providers/{provider}` (add `anthropic`), AES-256-GCM, server-only Firestore (`allow read,write: if false`), never returned to the client (last4 only). Generalize `/api/secrets/[provider]/{route,status,test}` and add a Settings "Model & Provider" section (per-provider key + test + default-model picker + honest per-model guidance) plus an active-model badge in the workflow. Invariants preserved: keys never client-side, allowlisted models only, MFA gate, prompt-size cap, and the GDPR `recursiveDelete providers/*` cascade already covers every provider. The platform free tier stays **Gemini**; **Claude is BYOK-only** (no free tier to subsidize). Key engineering nuance: normalize JSON output across providers (Gemini `responseMimeType:'application/json'` vs Anthropic `system` param + assistant `{` prefill) so the existing JSON parsers work unchanged. Deferred to **v2.x** — not a launch item; run the full 7-stage flow against Claude before shipping (prompts are Gemini-tuned). For exact model IDs / structured-output details, load the `claude-api` skill during implementation.
