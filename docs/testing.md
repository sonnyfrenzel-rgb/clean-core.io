...
*   **[`tests/credential-and-consent-guard.spec.ts`](file:///c:/Users/felix/antigravity/Project-Platform/tests/credential-and-consent-guard.spec.ts):** Asserts the S/4 vault stays all-or-nothing across all four outbound routes (the next one will be written by copying an existing one), and that terms acceptance is recorded only on the path that actually shows the checkboxes.
