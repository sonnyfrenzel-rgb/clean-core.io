...
- **Source change → stale artefacts** (E01-F01-US02) — when `runs/create` sees the source digest change, it writes, in the same batch as the `activeRunId` switch, `auditMetadata.sourceChange`: the digests of design, code, tests (without verdicts) and docs, and the sign-off timestamp, as they stood. Anything still carrying its recorded digest is stale until regenerated; `staleness()` / `generationBlockers()` / `handoverBlockers()` in `lib/workflow-steps.ts` read it, and `lib/artefact-digest.ts` (sync SHA-256, no imports) is shared by writer and readers so they cannot hash differently. `audit-pack/create` returns **409** with `blockers: ['source-changed' | 'sign-off-stale']`. No rules change was needed — `auditMetadata` is server-only. Not a tamper-evident chain (that is E01-F02).
...
- `coverage.ts` — `assessCoverage`, returned on the report as `coverage`. Records what the detectors did **not** judge (file I/O, local `CALL FUNCTION`, dynamic invocation, classic `WRITE` lists, macros, runtime-generated code) with line anchors. Deliberately beside `findings` and never inside it: an unassessed construct is a limit of the question, not a defect. Without it an empty finding list is indistinguishable from a clean program — which is how two starter examples came to be presented as spotless while calling BAPIs and writing files.
...
- `extensibility-router.ts` — `routeExtensibility` decides RAP (in-app) vs CAP (side-by-side) track. A write to the customer's own Z-table is **not** on its own a reason to leave the stack: in Private Edition / RISE that is the developer-extensibility case (Dictionary object + RAP business object, on-stack), and only Public Edition's strict SaaS model routes custom persistence Side-by-Side. The rule therefore reads `deploymentModel`, not the finding. `components/ArchitectSignOff.tsx` states the same rule for the reader, and `tests/extensibility-route-guard.spec.ts` fails if the two drift apart.
...
- Usage import: `usage-*.ts` — SCMON/UPL/ST03N parsing + `usage-privacy.ts` (pseudonymization). Since v2.9.7 (E03-F02) the importer **declares** the date format (`UsageDateLocale`) and the monitoring window; dates are built from their parts (`parseUsageDate`, never `new Date(text)`), bad rows go to `report.quarantined` with row and reason, and `UsageUpload` shows a preview and stores only on confirm. A zero count is `dormant` only across a declared window ≥ `RETIREMENT_WINDOW_DAYS` (394); otherwise `unobserved` → quadrant `unknown`. The join refuses mixed sources. The report must stay free of `undefined` — the Firestore client refuses it and the save used to fail silently.
...
- **Encryption** — S/4HANA creds AES-256-GCM at rest (`lib/s4-credentials.ts`, `S4_ENCRYPTION_KEY`), server-only Firestore collection `s4_credentials/{uid}` (`allow read, write: if false`). Every route that reaches a tenant asks `assertS4TenantAccess` — approval **and**, since 18.09.2026, an enrolled second factor (`s4AccessRequiresEnrolment`, `lib/mfa-gate.ts`; the admin claim does not exempt) — and `assertMfaSatisfied` for the factor on the token. The enrolment check is skipped only under the Firebase emulator, which cannot enrol TOTP; the decision is a pure function and tested without it. The own-key routes (`/api/secrets/gemini*`) require enrolment the same way via `assertMfaSatisfied(…, { requireEnrolment: byokRequiresEnrolment })`; every other MFA-gated route keeps the conditional gate (decision 18.09.2026, `tests/s4-mfa-enrolment-guard.spec.ts`). **There are no MFA backup codes** — roadmap 0.13 replaced the application-level TOTP with Firebase's own factor and deleted the codes, the pepper and `lib/mfa.ts` with it; a lost authenticator is recovered out of band via `scripts/mfa-reset.ts` (`SECURITY.md` §3.5). This line claimed peppered backup codes hashed in a module that no longer exists, and `/trust` repeated the claim to the public until 22.09.2026.
...
- **Env vars** — template in `.env.example`; runtime secrets injected as Cloud Run env vars per environment. `run-tests` does not execute generated tests itself: it sends them to the isolated runner service (`runner/`, roadmap 8.9 — service account without roles, no secrets, internal ingress, egress through a VPC without NAT; `RUNNER_URL`), checks the file hashes the runner reports, and fails closed without a runner outside an emulator build. The execution core (esbuild + Node Permission Model, temp-dir-scoped FS, minimal env, module and network guards, time/output limits) is `lib/test-sandbox/`, shared by the runner and the emulator-only local path. Live test execution against a tenant is **locked** (`LIVE_TEST_EXECUTION` in `lib/locked-paths.ts`, gate G0:R0, SECURITY.md §7.1); behind the lock a live run goes to the live runner with a short-lived capability, and the credential proxy `/api/s4-proxy` adds the tenant credentials per request. The old switch `S4_TEST_RUNNER_EGRESS_ENFORCED` is gone.

Required secrets: `GEMINI_API_KEY`, `RESEND_API_KEY`, `S4_ENCRYPTION_KEY`, `PILOT_APPROVAL_SECRET`, `AUDIT_SIGNING_KEY`, `S4_HOST_ALLOWLIST`.
